...
describe('buildObjectStorageViewModel', () => {
  it('formats summary, latency, cost, and raw events for rendering', () => {
    ...
    expect(vm.efficiency.find((metric) => metric.label === 'Avg S3 GET/HEAD request time')?.value).toBe('375ms');
    ...
    expect(vm.time[0]?.segments.find((segment) => segment.label === 'S3 open/init time')?.value).toBe('500.0ms');
    expect(vm.time[0]?.segments.find((segment) => segment.label === 'S3 open/init time')?.detail).toBe('Time spent initializing connection to S3.');
    expect(vm.time[0]?.segments.find((segment) => segment.label === 'Body streaming estimate')?.value).toBe('1.50s');
    expect(vm.time[0]?.segments.find((segment) => segment.label === 'Body streaming estimate')?.detail).toContain('Formula: ReadBufferFromS3Microseconds - ReadBufferFromS3InitMicroseconds');
    expect(vm.time[0]?.subtitle).toBe('Read-buffer elapsed time and summed HTTP request time are separate ClickHouse ProfileEvents.');
