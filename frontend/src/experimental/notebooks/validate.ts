 ...
 * text editor, and the renderer trusts its input completely: a cell naming an
...
export function validateNotebook(input: unknown): NotebookValidation {
  ...
  if (!Array.isArray(input.cells) || input.cells.length === 0) {
    error('cells', 'must contain at least one cell');
  }
  ...
  cells.forEach((raw, index) => {
    ...
    // Cell IDs become React keys and anchor hrefs, so duplicates break both.
    ...
    if (raw.columns !== undefined) {
      if (!Array.isArray(raw.columns) || raw.columns.length === 0) {
        error(`${at}.columns`, 'must be a non-empty array');
      } else {
        const fields = new Set<string>();
        raw.columns.forEach((column, columnIndex) => {
          const location = `${at}.columns[${columnIndex}]`;
          if (!isObject(column)) { error(location, 'must be an object'); return; }
          if (typeof column.field !== 'string' || !Array.isArray(item?.columns) || !item.columns.includes(column.field)) {
            error(location, 'must reference an evidence column');
          } else if (fields.has(column.field)) error(location, 'duplicates a display field');
          else fields.add(column.field);
          if (typeof column.label !== 'string' || !column.label.trim()) error(location, 'requires a readable label');
          if (column.type !== undefined && !['text', 'query', 'sql', 'bytes', 'timestamp'].includes(column.type as string)) {
            error(location, 'has an unsupported column type');
          }
        });
      }
    }
...
/** Parse and validate in one cell, so callers handle bad JSON the same way. */
