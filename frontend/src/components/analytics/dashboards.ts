...
const BUILTIN_DASHBOARDS: Dashboard[] = [
  ...
  {
    id: 'json-monitoring',
    title: 'Native JSON Monitoring',
    description: 'Track native JSON ingestion cost, materialized subcolumns, path pressure, and merge overhead',
    ...
    filters: [
      {
        param: 'tbl',
        label: 'JSON Table',
        query: "SELECT concat(database, '.', table) AS tbl FROM system.columns WHERE type LIKE 'JSON%' GROUP BY database, table ORDER BY tbl",
      },
    ],
    panels: [
      { queryName: 'JSON#JSON Columns Inventory', section: 'Schema & Path Pressure' },
      { queryName: 'JSON#JSON/String Storage Candidates' },
      { queryName: 'JSON#JSON Subcolumn Pressure' },
      { queryName: 'JSON#JSON Subcolumn Storage' },
      { queryName: 'JSON#JSON Column Storage by Part' },
      { queryName: 'JSON#JSON Insert Throughput', section: 'Write Path Cost' },
      { queryName: 'JSON#JSON Insert CPU & Memory' },
      { queryName: 'JSON#Active JSON Inserts' },
      { queryName: 'JSON#JSON Merge Duration per M rows', section: 'Merge Cost' },
      { queryName: 'JSON#JSON Merge Cost Breakdown' },
    ],
  },
  {
    id: 'select-perf',
  ...
  {
    id: 'self-monitoring',
    title: 'TraceHouse Query Cost',
    description:
      "Resources consumed by TraceHouse's own internal queries: cost per component, error rates, share of server load",
    group: 'TraceHouse',
    columns: 2,
    panels: [
      { queryName: 'Self-Monitoring#TraceHouse Query Duration by Component' },
      { queryName: 'Self-Monitoring#TraceHouse Query Volume by Component' },
      { queryName: 'Self-Monitoring#TraceHouse Query Timeline (5min buckets)' },
      { queryName: 'Self-Monitoring#TraceHouse Query Cost Details' },
      { queryName: 'Self-Monitoring#TraceHouse Query Duration Trend (hourly)' },
      { queryName: 'Self-Monitoring#TraceHouse % of Server Load' },
      { queryName: 'Self-Monitoring#Slowest TraceHouse Queries' },
      { queryName: 'Self-Monitoring#TraceHouse Failed Queries' },
    ],
  },
  {
  ...
  },
  {
    ...
    panels: [
      { queryName: 'Cloud Providers#ClickHouse Cloud Query Duration by User', section: 'Provider Overhead' },
      { queryName: 'Cloud Providers#ClickHouse Cloud Finished Queries by User' },
      { queryName: 'Cloud Providers#ClickHouse Cloud Query Timeline (5min)' },
      { queryName: 'Cloud Providers#Cloud Internal vs Other Query Activity' },
      { queryName: 'Cloud Providers#ClickHouse Cloud Query Cost Details', section: 'Query Shapes' },
      { queryName: 'Cloud Providers#Slowest ClickHouse Cloud Queries' },
      { queryName: 'Cloud Providers#Failed ClickHouse Cloud Queries' },
