...
ORDER BY elapsed DESC`,
  ...
  `-- @meta: title='Processes retrospectively' group='Altinity KB' interval='1 DAY' description='Who ate my CPU - query shapes ranked by user CPU time (UserTimeMicroseconds) from query_log. userCPUms is summed across all runs of the same shape; cpu_per_sec is userCPUms / query_duration_ms. Click a row to see individual executions.'
...
ORDER BY userCPUms DESC`,
  ...
  `-- @meta: title='CPU Query Executions' group='Altinity KB' interval='1 DAY' description='Individual executions of a selected query shape - drill target from Processes retrospectively. Click a query_id for the full query.'
...
ORDER BY val DESC`,
  ...
  `-- @meta: title='Top queries by peak memory' group='Altinity KB' description='Who ate my ClickHouse memory? - currently running queries sorted by peak memory usage. peak_mb (numeric MB) carries an inline bar relative to the biggest and is coloured by size; the KB columns are kept alongside.'
...
LIMIT 10`,
  ...
  `-- @meta: title='Top memory queries (history)' group='Altinity KB' interval='1 DAY' description='Who ate my ClickHouse memory? - completed queries from query_log sorted by memory usage.'
-- @source: https://kb.altinity.com/altinity-kb-setup-and-maintenance/altinity-kb-who-ate-my-memory/
SELECT
    type,
    event_time,
...
LIMIT 10`,
  ...
  `-- @meta: title='RAM peaks by hour (retrospection)' group='Altinity KB' interval='1 DAY' description='Who ate my ClickHouse memory? - retrospection analysis of RAM usage from part_log, query_log and query_views_log: peak RAM per hour, broken down by operation type (inserts, selects, merges, mutations, etc.).'
-- @source: https://kb.altinity.com/altinity-kb-setup-and-maintenance/altinity-kb-who-ate-my-memory/
WITH
...
ORDER BY timeframe`,
  ...
  `-- @meta: title='Thread pool limits (settings)' group='Altinity KB' description='Threads - the static, configured pool sizes from system.settings (the limits). Compare these against the current usage panel.'
...
ORDER BY name ASC`,
  ...
  `-- @meta: title='Thread pool usage (current)' group='Altinity KB' description='Threads - the live, in-use values aggregated from system.metrics and system.asynchronous_metrics: background pool tasks and current / async thread metrics, tagged by source.'
-- @source: https://kb.altinity.com/altinity-kb-setup-and-maintenance/altinity-kb-threads/
...
LIMIT 30`,
  ...
  `-- @meta: title='max_threads detection' group='Altinity KB' description='cgroups and k8s - the KB checks max_threads from system.settings; we also surface NumberOfPhysicalCPUCores for context. Under cgroup/k8s limits cores can be mis-detected, deflating CPU parallelism.'
...
UNION ALL SELECT 'NumberOfPhysicalCPUCores', toString(toInt64(value)) FROM system.asynchronous_metrics WHERE metric = 'NumberOfPhysicalCPUCores'`,
  ...
  `-- @meta: title='Minimum necessary set of checks' group='Altinity KB' description='ClickHouse Monitoring - the non-event single-value health checks the KB recommends (metrics and counts). The system.events error counters are in the Error Events panel.'
...
UNION ALL SELECT 'Uptime (sec)', toString(toInt64(coalesce((SELECT value FROM system.asynchronous_metrics WHERE metric = 'Uptime'), 0)))`,
  ...
  `-- @meta: title='Error Events' group='Altinity KB' description='ClickHouse Monitoring - all the system.events error counters from the KB checks (data loss, data-differs, rejected inserts, ZooKeeper / distributed connection failures, failed fetches). Shown only when non-zero; an empty panel means none have fired.'
...
ORDER BY value DESC`,
  ...
  `-- @meta: title='RAM vs max_server_memory_usage' group='Altinity KB' description='memory configuration settings - physical / cgroup RAM reported by the OS vs the configured server memory limit. ILIKE MemoryTotal surfaces the cgroup limit on k8s. Confirm the limit matches the host.'
...
LIMIT 30`,
  ...
  `-- @meta: title='Active Parts per Partition' group='Altinity KB' description='Number of active parts in a partition - partitions with the most active parts. High counts signal merge pressure / too-many-parts risk. Click a row to list its parts.'
-- @cell: column=part_count type=rag green<20 amber<100
-- @cell: column=part_count type=gauge max=max_parts
-- @drill: on=table into='Active Parts by Table'
...
LIMIT 20`,
  ...
  `-- @meta: title='Active Parts by Table' group='Altinity KB' description='Individual active parts for a selected table - drill target from Active Parts per Partition.'
-- @source: https://kb.altinity.com/altinity-kb-useful-queries/altinity-kb-number-of-active-parts-in-a-partition/
SELECT
    name,
    partition,
    ...
    modification_time
FROM system.parts
WHERE active AND table = {{drill_value:table | ''}}
...
LIMIT 100`,
  ...
  `-- @meta: title='Per-Disk Data Distribution' group='Altinity KB' description='MultiDisk (JBOD) Balancing - derived read-only view of how active data is spread across disks. Spot JBOD imbalance or a filling volume. (The KB article itself only provides a MOVE PART generator, which is excluded.)'
...
ORDER BY sum(bytes_on_disk) DESC`,
  ...
  `-- @meta: title='Part Lifecycle (30m)' group='Altinity KB' interval='1 DAY' description='Database Size - Table - Column size - per-second part-event rates (NewPart / MergeParts / DownloadPart / RemovePart / MutatePart / MovePart) per table over 30-minute buckets. The ingestion and merge rhythm.'
...
ORDER BY t ASC`,
  ...
  `-- @meta: title='Compression Ratio by Table' group='Altinity KB' description='How to test different compression codecs - uncompressed / compressed ratio per table. Low ratios may indicate poorly-ordered data or wrong codecs. Click a row for per-column detail.'
...
LIMIT 20`,
  ...
  `-- @meta: title='Compression by Column' group='Altinity KB' description='How to test different compression codecs - per-column size, ratio and codec for a selected table. Drill target from Compression Ratio by Table.'
-- @source: https://kb.altinity.com/altinity-kb-schema-design/codecs/altinity-kb-how-to-test-different-compression-codecs/
SELECT
    pc.column AS column,
    any(pc.type) AS type,
...
ORDER BY database, table`,
  ...
  `-- @meta: title='Replication Queue by Type' group='Altinity KB' description='Replication queue - the queue aggregated by table and task type, with executing / postponed / error counters and max retries.'
...
LIMIT 50`,
  ...
  `-- @meta: title='Distributed DDL Queue' group='Altinity KB' description='DDLWorker and DDL queue problems - read-only listing of ON CLUSTER DDL tasks that have not finished. A node lagging here means DDL is not fully applied across the cluster.'
...
LIMIT 50`,
  ...
  `-- @meta: title='Aggregated Stack Traces' group='Altinity KB' description='Debug hanging thing - thread stack traces grouped by frequency (address + symbol per frame, as the KB does). The fastest way to see what a busy or hung server is doing. Requires allow_introspection_functions.'
...
SETTINGS allow_introspection_functions = 1`,
  ...
  `-- @meta: title='Cluster connectivity probe' group='Altinity KB' description='Notes on errors (replication and distributed connections) - the KB Check Cluster Connectivity probe: actively fans out to every replica and counts how many respond.'
...
LIMIT 30`,
  ...
  `-- @meta: title='Cluster Connectivity' group='Altinity KB' description='Notes on errors (replication and distributed connections) - Check for Errors: cluster nodes reporting connection errors or slowdowns, per host. Points at DNS / network / down replicas.'
-- @source: https://kb.altinity.com/altinity-kb-useful-queries/connection-issues-distributed-parts/
SELECT
    hostName() AS host,
    cluster,
