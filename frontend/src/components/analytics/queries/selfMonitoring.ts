...
const queries: string[] = [
  `-- @meta: title='TraceHouse Query Duration by Component' group='Self-Monitoring' interval='1 DAY' description='Stacked percentile bands per component - bar height = p99, segments show p50 / p95−p50 / p99−p95'
-- @chart: type=stacked_bar group_by=component value=value_ms series=metric unit=ms style=2d
-- @drill: on=component into='TraceHouse Query Duration by Service'
...
ORDER BY component, metric`,
  ...
  `-- @meta: title='TraceHouse Query Duration by Service' group='Self-Monitoring' interval='1 DAY' description='Drill from component to individual services - stacked percentile bands per service'
-- @chart: type=stacked_bar group_by=service value=value_ms series=metric unit=ms style=2d
-- @drill: on=service into='TraceHouse Query Cost Details'
...
ORDER BY service, metric`,
  ...
  `-- @meta: title='TraceHouse Query Volume by Component' group='Self-Monitoring' interval='1 DAY' description='Number of queries fired by each TraceHouse component in the selected time range'
-- @chart: type=pie group_by=component value=query_count style=3d
-- @drill: on=component into='TraceHouse Query Volume by Service'
SELECT
    extractAllGroups(query, '${APP_RE_COMPONENT}')[1][1] AS component,
...
ORDER BY query_count DESC`,
  ...
  `-- @meta: title='TraceHouse Query Volume by Service' group='Self-Monitoring' interval='1 DAY' description='Drill from component to individual services - query count per service'
...
ORDER BY query_count DESC`,
  ...
  `-- @meta: title='TraceHouse Query Cost Details' group='Self-Monitoring' interval='1 DAY' description='Per-query-shape cost breakdown - duration, peak memory statistics, rows, bytes, and CPU'
...
ORDER BY total_memory_mb DESC`,
  ...
  `-- @meta: title='TraceHouse Query Executions' group='Self-Monitoring' interval='1 DAY' description='Recent executions for a specific query shape - click a row to open the full Query Detail view'
...
LIMIT 50`,
  ...
  `-- @meta: title='TraceHouse Query Timeline (5min buckets)' group='Self-Monitoring' interval='6 HOUR' description='TraceHouse query rate and avg duration over time - spot polling spikes'
...
ORDER BY t ASC`,
  ...
  `-- @meta: title='Slowest TraceHouse Queries' group='Self-Monitoring' interval='1 DAY' description='Top 20 slowest individual queries generated by TraceHouse'
...
LIMIT 20`,
  ...
  `-- @meta: title='TraceHouse Memory & Rows by Component' group='Self-Monitoring' interval='1 DAY' description='Sum of per-query peak memory plus rows read per component - find the heaviest hitters'
...
ORDER BY total_memory_mb DESC`,
  ...
  `-- @meta: title='TraceHouse Failed Queries' group='Self-Monitoring' interval='7 DAY' description='TraceHouse queries that threw exceptions - helps catch broken polling or bad SQL'
-- @link: on=query_id into='Query Detail by ID'
SELECT
    event_time,
    query_id,
...
LIMIT 30`,
  ...
  `-- @meta: title='TraceHouse Query Duration Trend (hourly)' group='Self-Monitoring' interval='2 DAY' description='Hourly p50/p95/p99 of TraceHouse query duration - detect regressions over time'
...
ORDER BY host`,
  ...
  `-- @meta: title='TraceHouse % of Server Load' group='Self-Monitoring' interval='1 DAY' description='What fraction of total server query time is consumed by TraceHouse itself'
