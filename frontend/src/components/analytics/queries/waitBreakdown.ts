...
ORDER BY rows_bucket ASC, query_shape`,
  ...
  `-- @meta: title='Worst Waiting Queries' group='Wait Breakdown' interval='6 HOUR' description='Individual executions over 100ms, ranked by thread time spent NOT working (queue + disk + network + parked) — the queries whose cost is waiting, not the ones that merely ran longest. Bars are absolute thread-milliseconds, so lengths are comparable between queries, unlike the proportional panels above. Click a bar to open that execution; a shard execution opens its coordinator on the Distributed tab. Drilled in from a shape, this lists that shape\\'s own executions, so the composition matches the bar you clicked.'
