...
module.exports = (env) => {
  ...
  return {
    ...
    output: {
      ...
      publicPath: '/public/plugins/dmkskd-tracehouse-app/',
      uniqueName: 'dmkskd-tracehouse-app',
      // The Grafana plugin-validator joins sourcemap paths against
      // <sourceCodeDir>/src/, so paths must already be relative to src/.
      // Default webpack output is `./src/module.ts` which validator joins as
      // `<sourceCodeDir>/src/src/module.ts` — wrong. Strip the `./src/` prefix
      // for plugin files; leave external (`../frontend/`, `../packages/`)
      // paths alone since the `..` in filepath.Join already resolves correctly.
      devtoolModuleFilenameTemplate: (info) => {
        let p = info.resourcePath;
        if (p.startsWith('./src/')) p = p.substring('./src/'.length);
    ...
    resolve: {
      ...
      alias: {
        ...
        // Redirect URL param reader to use locationService instead of window.location
