...
:::note Platform
This project has only been tested on macOS (Apple Silicon / ARM). Other platforms should work but are not officially supported.
:::
...
Override the image for a single run when testing another ClickHouse release:
...
You can also [try TraceHouse online](https://dmkskd.github.io/tracehouse-assets/tracehouse.html) without installing anything. It is a single-file HTML that runs entirely in your browser with any [CORS-enabled](./guides/connecting#cors-proxy) ClickHouse instance.

The online version will ask for your ClickHouse credentials. If you prefer not to enter them on a third-party page, download the HTML file from the [releases page](https://github.com/dmkskd/tracehouse/releases/latest) and open it locally.
...
just generate-data-quick    # Small dataset, faster
```
