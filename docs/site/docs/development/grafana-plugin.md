...
TraceHouse ships as a Grafana **app plugin** (`tracehouse-app`) that reuses the standalone frontend — same page components, same shared packages — packaged with the plugin scaffolding (`plugin.json`, `module.ts`, webpack config, and adapter shims) needed to run inside Grafana's sandbox. The standalone app connects to ClickHouse directly (via the built-in proxy or user-provided credentials), while the Grafana plugin routes all queries through Grafana's ClickHouse datasource plugin — meaning ClickHouse connectivity is managed by Grafana's datasource configuration, not by TraceHouse itself. All pages — Overview, Engine Internals, Cluster, Explorer, Time Travel, Queries, Merges, Replication, Analytics — are available as Grafana navigation entries.
...
- **Pages:** Declared in `plugin.json` under `includes[]` — each page gets a Grafana nav entry at `/a/tracehouse-app/<slug>`
