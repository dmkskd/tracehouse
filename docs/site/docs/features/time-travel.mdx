...
The event rail below the detailed chart shows query resource failures, DDL,
server lifecycle events, and background/storage failures alongside the
continuous metrics. Filters can include or exclude severities, categories, or
individual event types.

Select an event to pin its timestamp and see which queries, merges, and
mutations were running then. Use **View details** to open the top-level
**Events** page, including its source evidence and nearby events.
