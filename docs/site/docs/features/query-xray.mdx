...
```bash
./infra/scripts/setup_processes_sampling.sh
```
...
./infra/scripts/setup_processes_sampling.sh --dry-run

# Custom host and credentials
./infra/scripts/setup_processes_sampling.sh --host my-ch-node --user admin --password secret

# Custom sampling interval (default: 1s) and retention (default: 7 days)
./infra/scripts/setup_processes_sampling.sh --interval 5 --ttl 14
```

See `./infra/scripts/setup_processes_sampling.sh --help` for all options.
