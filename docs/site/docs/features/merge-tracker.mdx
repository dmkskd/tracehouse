...
Active merge row counts use `system.merges.rows_read`, which is cumulative progress while the operation is running. Historical merge rows use `system.part_log.read_rows`, which is the completed operation's final input-row count.

Lightweight `UPDATE` patch parts are visible through active mutation/merge state, but patch-part creation is not represented as a normal historical merge event in `system.part_log`. The historical merge list can classify classic mutations and lightweight-delete cleanup merges, but it cannot reconstruct every patch-part creation event from `part_log` alone.
...
<FeatureVideo src="merge-health-1920.mp4" />
