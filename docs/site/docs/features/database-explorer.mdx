...
## Example: Exploring a Full Merge

When `OPTIMIZE TABLE ... FINAL` is invoked, ClickHouse forces a full compaction, merging all parts within each partition down to a single part. As you can see from the video, it is a very expensive operation that rewrites all data in the table.

<FeatureVideo src="optimize-table-final-1920.mp4" />
